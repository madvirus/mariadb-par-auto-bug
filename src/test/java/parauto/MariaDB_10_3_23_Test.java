...
@Testcontainers
public class MariaDB_10_3_23_Test extends MariaDBTestBase {
    private Logger logger = LoggerFactory.getLogger(getClass());

    @Container
    public MariaDBContainer mariaDB = (MariaDBContainer) new MariaDBContainer("mariadb:10.3.23")
            .withConfigurationOverride("conf.d")
            .withInitScript("init.sql");

    @Test
    void test10_3_23() throws SQLException, InterruptedException {
        ...
        logger.info("id5 추가 결과: error={}, errorMessage={}", result.isError(), result.getErrorMessage());

        logMonthVisitors("id5 추가 실패 후");
        logger.info("next AUTO_INCREMENT: {}", selectAutoInc());

        runWithTx(conn -> {
            insertMonthVisitor(conn, 202108, "id6");
        });
        logMonthVisitors("id6 추가 후");
        logger.info("next AUTO_INCREMENT: {}", selectAutoInc());
    }
